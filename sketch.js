...
function draw() {
  ...
  if(boy.isTouching(wGroup)){
    score=7;
    pGroup.destroyEach();
    whiteHat.velocityX=0;
    whiteHat.x=boy.x;
    whiteHat.y=boy.y-100;
  }

  //space key 
  if(keyDown("space") && boy.y>=495){
    jumpSound.play();
    boy.velocityY=-15;
  }
  ...
  if(boy.isTouching(obstacleGroup)){
   ...
   //displaying life
   /*if(life===3){
    text("Chances",100,100);
    image(heart,150,100,10,10);
    image(heart,170,100,10,10);
    image(heart,190,100,10,10);
